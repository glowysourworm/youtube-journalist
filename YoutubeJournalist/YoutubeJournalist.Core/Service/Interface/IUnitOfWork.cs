...
namespace YoutubeJournalist.Core.Service.Interface
{
    ...
    public interface IUnitOfWork : IDisposable
    {
        ...
        /// </summary>
        /// <returns></returns>
        IEnumerable<Youtube_SearchResult> GetSearchResults();

        /// <summary>
        /// Returns entire set of channel entities from local database
        /// </summary>
        IEnumerable<Youtube_Channel> GetChannels();
        ...
        /// </summary>
        IEnumerable<Youtube_Video> GetVideos();

        /// <summary>
        /// Returns set of videos for a specific channel from local database
        /// </summary>
        IEnumerable<Youtube_Video> GetVideos(string channelId);
        ...
        /// </summary>
        IEnumerable<Youtube_Video> SearchUpdateVideoDetails(YoutubeVideoDetailsRequest request);

        /// <summary>
        /// Search that retrieves comment threads for:  1) An entire channel, or 2) A set of video (ids)
        /// </summary>
        IEnumerable<Youtube_CommentThread> SearchCommentThreads(YoutubeCommentThreadRequest request);
    }
}
